...
export function ReportLostForm({ onDone }: { onDone?: () => void }) {
  ...
  const submitForm = (event: React.FormEvent) => {
    ...
    if (!itemType.trim() || contactName.trim().length < 2 || !phoneValid) { toast.error("Add the item, your name and a valid phone number."); setStep(4); return; }
    mutation.mutate();
  };
  ...
  return (
    <form onSubmit={submitForm} className="space-y-5">
      ...
      <div className="hidden grid-cols-5 gap-2 md:grid">{STEPS.map((label, index) => <div key={label} className="flex items-center gap-2 text-[11px] font-bold text-muted-foreground"><span className={`grid h-6 w-6 place-items-center rounded-full border ${index <= step ? "border-gold bg-gold text-background" : "border-border/70"}`}>{index < step ? <Check className="h-3.5 w-3.5" /> : index + 1}</span>{label}</div>)}</div>
      <AnimatePresence mode="wait" initial={false}>
        <motion.div key={step} initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -14 }} transition={{ duration: 0.2 }} className="premium-surface min-h-[20rem] rounded-[2rem] border border-border/60 bg-card/65 p-5 shadow-lift sm:p-8">
          {step === 0 && <StepShell icon={Package} eyebrow="Step 01 · The item" title="What did you lose?" helper="Start with the clearest name you can give the item."><CategoryChips value={category} onChange={setCategory} /><Field label="Item name" required><Input autoFocus value={itemType} onChange={(event) => setItemType(event.target.value)} placeholder="Samsung A14 phone" className="h-12 rounded-xl bg-secondary/40 text-[15px]" /></Field></StepShell>}
          {step === 1 && <StepShell icon={MapPin} eyebrow="Step 02 · Where & when" title="Where did it happen?" helper="A landmark or district helps officers search faster."><div className="grid gap-4 sm:grid-cols-2"><Field label="Place description"><Input value={locationText} onChange={(event) => setLocationText(event.target.value)} placeholder="Kikuubo, near the taxi stage" className="h-12 rounded-xl bg-secondary/40 text-[15px]" /></Field><Field label="District"><Input value={district} onChange={(event) => setDistrict(event.target.value)} placeholder="Kampala" className="h-12 rounded-xl bg-secondary/40 text-[15px]" /></Field><Field label="Date lost"><Input type="date" value={occurredOn} onChange={(event) => setOccurredOn(event.target.value)} className="h-12 rounded-xl bg-secondary/40 text-[15px]" /></Field></div></StepShell>}
          {step === 2 && <StepShell icon={FileText} eyebrow="Step 03 · Details" title="Help us recognise it" helper="Include colour, marks, contents, or any serial number you remember."><Field label="Description"><Textarea autoFocus value={description} onChange={(event) => setDescription(event.target.value.slice(0, 1500))} rows={7} placeholder="Black case, cracked corner, documents inside..." className="resize-none rounded-2xl bg-secondary/40 text-[15px] focus-visible:ring-gold/40" /><div className="mt-2 text-right text-[11px] font-semibold tabular-nums text-muted-foreground">{description.length} / 1500</div></Field></StepShell>}
          {step === 3 && <StepShell icon={ImagePlus} eyebrow="Step 04 · Photo" title="Add a photo if you have one" helper="A photo can help matching, but this step is completely optional."><PhotoStep inputRef={fileInputRef} preview={photoPreview} onPick={pickPhoto} onRemove={removePhoto} /></StepShell>}
          {step === 4 && <StepShell icon={UserRound} eyebrow="Step 05 · Your contact" title="Where can officers reach you?" helper="Your details stay private and are shown only to verified officers."><div className="grid gap-4 sm:grid-cols-2"><Field label="Full name" required><Input autoFocus value={contactName} onChange={(event) => setContactName(event.target.value)} placeholder="Full name" className="h-12 rounded-xl bg-secondary/40 text-[15px]" /></Field><Field label="Phone" required><Input value={contactPhone} onChange={(event) => setContactPhone(event.target.value)} placeholder="0774 620 951" inputMode="tel" className={`h-12 rounded-xl bg-secondary/40 text-[15px] ${contactPhone && !phoneValid ? "border-destructive" : ""}`} /></Field></div>{contactPhone && !phoneValid && <p className="mt-2 text-xs font-semibold text-destructive">Enter a valid phone number.</p>}<ReviewSummary itemType={itemType} locationText={locationText} district={district} occurredOn={occurredOn} description={description} photo={Boolean(photo)} contactName={contactName} contactPhone={contactPhone} /></StepShell>}
        </motion.div>
      </AnimatePresence>
