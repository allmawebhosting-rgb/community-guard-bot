...
export function FeatureShowcase({ feature }: { feature: FeatureLink }) {
  return <section className="my-14 overflow-hidden border-y border-[#171817]/10 bg-[#171817] text-white"><div className="grid lg:grid-cols-[1.08fr_0.92fr]"><div className="p-6 sm:p-10"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#e2b84c]">{feature.category}</p><h3 className="mt-3 font-display text-3xl font-black tracking-[-0.04em] sm:text-4xl">{feature.name}</h3><p className="mt-4 max-w-lg text-base leading-7 text-white/70">{feature.description}</p><ul className="mt-7 space-y-3 text-sm text-white/80">{feature.benefits.map((benefit) => <li key={benefit} className="flex gap-3"><Check className="mt-0.5 h-4 w-4 shrink-0 text-[#e2b84c]" />{benefit}</li>)}</ul><a href={feature.href} className="mt-8 inline-flex min-h-11 items-center gap-2 border-b border-[#e2b84c] pb-1 text-xs font-black uppercase tracking-[0.16em] text-white">Explore {feature.name}<ArrowUpRight className="h-4 w-4" /></a></div><figure className="order-first min-h-[260px] bg-[#252624] lg:order-last"><img src={feature.image} alt={`${feature.name} feature illustration`} loading="lazy" className="h-full min-h-[260px] w-full object-cover opacity-90" /></figure></div></section>;
}

export function FeatureSplit({ feature, reverse = false }: { feature: FeatureLink; reverse?: boolean }) {
  return <section className={`my-14 grid items-center gap-8 lg:grid-cols-2 ${reverse ? "lg:[&>*:first-child]:order-2" : ""}`}><div className="overflow-hidden bg-[#e8e3da]"><img src={feature.image} alt={`${feature.name} in the Allma safety system`} loading="lazy" className="aspect-[4/3] w-full object-cover" /></div><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#c63d3f]">Related Allma feature</p><h3 className="mt-3 font-display text-3xl font-black tracking-[-0.04em]">{feature.name}</h3><p className="mt-4 text-base leading-7 text-[#5d6057]">{feature.description}</p><a href={feature.href} className="mt-6 inline-flex min-h-11 items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[#c63d3f]">Explore feature <ArrowUpRight className="h-4 w-4" /></a></div></section>;
}
