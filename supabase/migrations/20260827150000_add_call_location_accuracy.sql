-- Include captured GPS accuracy in the authorized emergency call context.
...
AS $$
  ...
  LEFT JOIN public.safety_connections sc ON sc.owner_id = c.caller_id AND sc.member_id = auth.uid()
  WHERE c.id = p_call_id AND c.recipient_id = auth.uid();
$$;
