  ...
  return (
    <div className="w-full space-y-4">
          ...
          <div className="max-w-2xl">
            <div className="mb-5 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-foreground/80">Authorized Allma operations workspace · {officer?.jurisdiction_area ?? "Uganda"}</p>
              </div>
            </div>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border/50 bg-card/60 px-3 py-2.5 shadow-soft">
        <span className="mr-1 text-[10px] font-bold uppercase tracking-[0.16em] text-foreground/75">Operational controls</span>
        <button type="button" onClick={() => setDemoMode((value) => !value)} className={cn("inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11px] font-semibold transition shadow-sm", demoMode ? "border-gold/60 bg-gold/14 text-gold" : "border-border/70 bg-secondary/70 text-foreground/90 hover:border-gold/40 hover:text-gold")}>
          {demoMode ? <Pause className="h-3 w-3" /> : <Play className="h-3 w-3" />}
          {demoMode ? "DEMO SIMULATION ACTIVE" : "Run DEMO simulation"}
        </button>
        <button type="button" onClick={() => setSurgeMode((value) => !value)} className={cn("inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11px] font-semibold transition shadow-sm", surgeMode ? "border-primary/60 bg-primary/12 text-primary" : "border-border/70 bg-secondary/70 text-foreground/90 hover:border-primary/40 hover:text-primary")}>
          <BellRing className="h-3 w-3" /> {surgeMode ? "SURGE MODE ON" : "Activate surge mode"}
        </button>
        <span className="ml-auto flex items-center gap-1.5 text-[10px] text-foreground/80">
        ...
        <section className="card-desktop overflow-hidden">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-border/40 pb-3">
            <div>
              ...
              <p className="mt-0.5 text-[10px] text-foreground/75">Real case state · choose a queue to review</p>
            </div>
            <Link to="/police/incidents" className="flex items-center gap-1 text-[11px] text-foreground/75 hover:text-foreground">View all <ArrowUpRight className="h-3 w-3" /></Link>
          </div>
        ...
        <div className="space-y-4">
          ...
          <section className="card-desktop">
            <div className="mb-3 flex items-center justify-between border-b border-border/40 pb-3">
              ...
              <Link to="/police/officers" className="text-[11px] text-foreground/75 hover:text-foreground">Manage <ArrowUpRight className="ml-0.5 inline h-3 w-3" /></Link>
            </div>
...
function QueueRow({ incident }: { incident: Incident }) {
  ...
  return (
    <Link to="/police/cases/$caseId" params={{ caseId: incident.id }} className="group flex items-start gap-3 py-2.5 transition hover:bg-secondary/20 -mx-4 px-4">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-[13px] font-semibold leading-snug text-foreground group-hover:text-primary">{incident.title}</p>
        <p className="mt-0.5 flex items-center gap-2 text-[11px] text-foreground/70">
      ...
      <span className="shrink-0 text-[11px] text-foreground/70">{timeAgo(incident.created_at)}</span>
    </Link>
  );
}

function ModeNotice({ icon: Icon, title, text, tone }: { icon: typeof Play; title: string; text: string; tone: "gold" | "primary" }) {
  return <div className={cn("flex items-start gap-3 rounded-2xl border px-4 py-3", tone === "gold" ? "border-gold/35 bg-gold/[0.08]" : "border-primary/40 bg-primary/[0.08]")}><Icon className={cn("mt-0.5 h-4 w-4 shrink-0", tone === "gold" ? "text-gold" : "text-primary")} /><div><p className={cn("text-xs font-semibold", tone === "gold" ? "text-gold" : "text-primary")}>{title}</p><p className="mt-0.5 text-[11px] leading-relaxed text-foreground/75">{text}</p></div></div>;
}

function AvailabilityCount({ label, value, tone }: { label: string; value: number; tone: "success" | "gold" | "muted" }) {
  return <div className="rounded-xl border border-border/60 bg-secondary/60 px-2 py-2 text-center"><p className={cn("text-[16px] font-bold tabular-nums text-foreground", tone === "success" ? "text-success" : tone === "gold" ? "text-gold" : "text-foreground/70")}>{value}</p><p className="text-[9px] uppercase tracking-wide text-foreground/70">{label}</p></div>;
}

function PriorityLane({ title, description, incidents, empty }: { title: string; description: string; incidents: Incident[]; empty: string }) {
  return <section className="premium-surface overflow-hidden rounded-3xl border border-border/60 shadow-soft"><div className="flex items-center justify-between border-b border-border/50 px-5 py-4"><div><h2 className="font-display text-sm font-semibold text-foreground">{title}</h2><p className="mt-0.5 text-[11px] text-foreground/70">{description}</p></div><span className="rounded-full border border-primary/30 bg-primary/12 px-2 py-1 text-[10px] font-semibold tabular-nums text-primary">{incidents.length} open</span></div><div className="divide-y divide-border/35 px-5">{incidents.map((incident) => <Link key={incident.id} to="/police/cases/$caseId" params={{ caseId: incident.id }} className="group flex items-center gap-3 py-3"><span className={cn("h-2 w-2 shrink-0 rounded-full", PRIORITY_META[incident.priority as IncidentPriority]?.dot ?? "bg-border")} /><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-foreground group-hover:text-primary">{incident.title}</p><p className="mt-1 text-[10px] text-foreground/70">{incident.reference} · {incident.district ?? "Unassigned"} · {timeAgo(incident.created_at)}</p></div><ChevronRight className="h-3.5 w-3.5 text-foreground/70" /></Link>)}{incidents.length === 0 && <p className="py-7 text-center text-xs text-foreground/70">{empty}</p>}</div></section>;
}

function BriefMetric({ label, value, tone }: { label: string; value: number; tone: "critical" | "reported" }) {
  return <div className={cn("rounded-2xl border bg-secondary/60 p-3 shadow-sm", tone === "critical" ? "border-primary/35" : "border-gold/35")}><p className="text-[10px] uppercase tracking-[0.12em] text-foreground/75">{label}</p><p className={cn("mt-1 font-display text-[2.1rem] font-black leading-none tracking-[-0.05em] tabular-nums text-foreground", tone === "critical" ? "text-primary" : "text-gold")}>{value}</p></div>;
}

function Stat({ label, value, icon: Icon, tone }: { label: string; value: number; icon: typeof Siren; tone: string }) {
  return <div className="card-desktop"><div className="flex items-center justify-between"><p className="label-xs text-foreground/75">{label}</p><Icon className={cn("h-3.5 w-3.5", tone)} /></div><p className="mt-1.5 font-display text-[2.1rem] font-black leading-none tracking-[-0.05em] tabular-nums text-foreground">{value}</p></div>;
}